...
from .util import stderr, lowerbound


def byte2bit(s):
    return b''.join(
        [b'%c' % (((s[i >> 3]) >> (7 - i & 7)) & 1) for i in range(len(s) * 8)])
...
class RFBProxy:
    ...
    def preferred_format(self, bitsperpixel, depth, bigendian, truecolour, red_max,
                         green_max, blue_max, red_shift, green_shift, blue_shift):
    ...
    def init(self):
        ...
        if self.debug:
            stderr('protocol_version: 3.%d' % self.protocol_version)
        return self
    ...
    def auth(self):
        ...
        def crauth():
            ...
            if self.debug:
                stderr('challange: %r' % challange)
            response = generate_response(p, challange)
            if self.debug:
                stderr('response: %r' % response)
            self.send(response)
        ...
        if self.protocol_version == 3:
            ...
            if self.debug:
                stderr('server_security: %r' % server_security)
            ...
            if self.debug:
                stderr('server_securities: %r' % server_securities)
        ...
        # result returned.
        if self.debug:
            stderr('server_result: %r' % server_result)
        if server_result != 0:
            # auth failed.
            if self.protocol_version != 3:
                (reason_length, ) = unpack('>L', self.recv(4))
    ...
    def start(self):
        ...
        (bitsperpixel, depth, bigendian, truecolour, red_max, green_max, blue_max,
         red_shift, green_shift, blue_shift) = unpack('>BBBBHHHBBBxxx', pixelformat)
        if self.debug:
            stderr('Server Encoding:')
            stderr(' width=%d, height=%d, name=%r' % (width, height, self.name))
            stderr(' pixelformat=', (bitsperpixel, depth, bigendian, truecolour))
            stderr(' rgbmax=', (red_max, green_max, blue_max))
            stderr(' rgbshift=', (red_shift, green_shift, blue_shift))
        # setformat
        self.send(b'\x00\x00\x00\x00')
        # 32bit, 8bit-depth, big-endian(RGBX), truecolour, 255max
        (bitsperpixel, depth, bigendian, truecolour, red_max, green_max, blue_max,
         red_shift, green_shift,
         blue_shift) = self.preferred_format(bitsperpixel, depth, bigendian, truecolour,
                                             red_max, green_max, blue_max, red_shift,
    ...
    def loop1(self):
        ...
        if c == b'':
            ...
            if self.debug:
                stderr('FrameBufferUpdate: nrects=%d' % nrects)
            for rectindex in range(nrects):
                (x0, y0, width, height, t) = unpack('>HHHHl', self.recv_relay(12))
                if self.debug:
                    stderr(' %d: %d x %d at (%d,%d), type=%d' %
                           (rectindex, width, height, x0, y0, t))
                # RawEncoding
                if t == 0:
                    l = width * height * self.bytesperpixel
                    data = self.recv_relay(l)
                    if self.debug:
                        stderr(' RawEncoding: len=%d, received=%d' % (l, len(data)))
                    if self.fb:
                ...
                # RREEncoding
                    ...
                    if self.debug:
                        stderr(' RREEncoding: subrects=%d, bgcolor=%r' %
                               (nsubrects, bgcolor))
                    if self.fb:
                    ...
                    for i in range(nsubrects):
                        ...
                        if 2 <= self.debug:
                            stderr(' RREEncoding: ', (x, y, w, h, fgcolor))
                # CoRREEncoding
                elif t == 4:
                    (nsubrects, ) = unpack('>L', self.recv_relay(4))
                    bgcolor = self.recv_relay(self.bytesperpixel)
                    if self.debug:
                        stderr(' CoRREEncoding: subrects=%d, bgcolor=%r' %
                               (nsubrects, bgcolor))
                    if self.fb:
                        self.fb.process_solid(x0, y0, width, height, bgcolor)
                    for i in range(nsubrects):
                        ...
                        if self.fb:
                            self.fb.process_solid(x0 + x, y0 + y, w, h, fgcolor)
                        if 2 <= self.debug:
                            stderr(' CoRREEncoding: ', (x, y, w, h, fgcolor))
                # HextileEncoding
                elif t == 5:
                    if self.debug:
                        stderr(' HextileEncoding')
                    ...
                    for y in range(0, height, 16):
                        for x in range(0, width, 16):
                            ...
                            if not c & 8:
                                if 2 <= self.debug:
                                    stderr('  Solid:', repr(bgcolor))
                                continue
                            nsubrects = ord(self.recv_relay(1))
                            # SubrectsColoured
                            if c & 16:
                                if 2 <= self.debug:
                                    stderr('  SubrectsColoured:', nsubrects,
                                           repr(bgcolor))
                                for i in range(nsubrects):
                                    color = self.recv_relay(self.bytesperpixel)
                                    (xy, wh) = unpack('>BB', self.recv_relay(2))
                                    if self.fb:
                                        self.fb.process_solid(
                                            x0 + x + (xy >> 4), y0 + y + (xy & 15),
                                            (wh >> 4) + 1, (wh & 15) + 1, color)
                                    if 3 <= self.debug:
                                        stderr('   ', repr(color), (xy, wh))
                            # NoSubrectsColoured
                            else:
                                if 2 <= self.debug:
                                    stderr('  NoSubrectsColoured:', nsubrects,
                                           repr(bgcolor))
                                for i in range(nsubrects):
                                    (xy, wh) = unpack('>BB', self.recv_relay(2))
                                    if self.fb:
                ...
                # RichCursor
                    ...
                    if width and height:
                        ...
                        if self.debug:
                            stderr('RichCursor: %dx%d at %d,%d' %
                                   (width, height, x0, y0))
                        if self.fb:
                            data = self.fb.convert_pixels(data)
                            mask = b''.join([
                                byte2bit(mask[p:p + rowbytes])[:width]
                            ...
                            data = b''.join([conv1(i) for i in range(0, len(data), 4)])
                ...
                # XCursor
                    ...
                    if width and height:
                        ...
                        if self.debug:
                            stderr('XCursor: %dx%d at %d,%d' % (width, height, x0, y0))
                        if self.fb:
                            data = b''.join([
                                byte2bit(data[p:p + rowbytes])[:width]
                                for p in range(0, height * rowbytes, rowbytes)
                            ])
                            mask = b''.join([
                                byte2bit(mask[p:p + rowbytes])[:width]
            ...
            if self.debug:
                stderr('SetColourMapEntries: first=%d, ncolours=%d' % (first, ncolours))
            for i in ncolours:
                self.recv_relay(6)
...
class RFBNetworkClient(RFBProxy):
    ...
    def __init__(self,
                 ...
                 fb=None,
                 pwdfile=None,
                 preferred_encoding=(0, 5),
                 debug=0):
        RFBProxy.__init__(self,
                          fb=fb,
                          pwdfile=pwdfile,
    ...
    def init(self):
        ...
        stderr('Connected: %s:%d, protocol_version=3.%d, preferred_encoding=%s' % (self.host, self.port, self.protocol_version, self.preferred_encoding))
        return x
    ...
    def request_update(self):
        if self.debug:
            stderr('FrameBufferUpdateRequest')
...
class RFBNetworkClientForRecording(RFBNetworkClient):
    ...
    def __init__(self, host, port, fp, pwdfile=None, preferred_encoding=(5, 0),
                 debug=0):
        RFBNetworkClient.__init__(self,
                                  host,
                                  port,
                                  fb=None,
                                  pwdfile=pwdfile,
                                  preferred_encoding=preferred_encoding,
                                  debug=debug)
        stderr('Creating vncrec: %r: vncLog0.0' % fp)
        self.fp = fp
        self.write(b'vncLog0.0')
        # disguise data (security=none)
        self.write(b'RFB 003.003\x0a')
...
class RFBFileParser(RFBProxy):
    ...
    def preferred_format(self, bitsperpixel, depth, bigendian, truecolour, red_max,
                         green_max, blue_max, red_shift, green_shift, blue_shift):
        if (bitsperpixel, depth, bigendian, truecolour, red_max, green_max, blue_max,
                red_shift, green_shift, blue_shift) == self.FASTEST_FORMAT:
            return RFBProxy.preferred_format(self, bitsperpixel, depth, bigendian,
                                             truecolour, red_max, green_max, blue_max,
            ...
            try:
                ...
                raise 'invalid {red,green,blue}_max: %d, %d or %d' % (
                    red_max, green_max, blue_max)
            getrgb = eval(e)
            unpack_pixels = eval(
                'lambda data: unpack("%s%%d%s" %% (len(data)/%d), data)' %
                (endian, length, nbytes))
            unpack_color1 = eval('lambda data: unpack("%s", data)' % unpackstr)
            self.fb.set_converter(
                lambda data: ''.join([
                    pack('>BBB', *getrgb(p)) for p in unpack_pixels(data)
                ]), lambda data: getrgb(unpack_color1(data)[0]))
        return (bitsperpixel, depth, bigendian, truecolour, red_max, green_max,
                blue_max, red_shift, green_shift, blue_shift)
    ...
    def init(self):
        ...
        stderr('Reading vncrec file: %s, version=%r...' % (self.fp, version))
...
class RFBConverter(RFBFrameBuffer):
    ...
    def init_screen(self, width, height, name):
        stderr('VNC Screen: size=%dx%d, name=%r' % (width, height, name))
    ...
    def process_pixels(self, x, y, width, height, data):
        self.images.append(
            ((x, y), (width, height, (IMG_RAW, self.convert_pixels(data)))))

    def process_solid(self, x, y, width, height, data):
        self.images.append(
            ((x, y), (width, height, (IMG_SOLID, self.convert_color1(data)))))
        ...
        if not self.processing:
            ...
            while len(self.frameinfo) < frames:
                if done:
                    ...
                    if self.debug:
                        stderr('scan:', self.beginpos, endpos)
